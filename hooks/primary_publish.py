...
class PrimaryPublishHook(Hook):
    ...
    def execute(self, task, work_template, comment, thumbnail_path, sg_task, progress_cb, **kwargs):
        ...
        if engine_name == "tk-maya":
            ...
            return self._do_syntheyes_publish(task, work_template, comment, thumbnail_path, sg_task, progress_cb)
        else:
    ...
    def _do_syntheyes_publish(self, task, work_template, comment, thumbnail_path, sg_task, progress_cb):
        """
        Publish the main SynthEyes scene
        ...
        progress_cb(0.0, "Finding scene dependencies", task)
        dependencies = self._syntheyes_find_additional_scene_dependencies()

        # get scene path
        hlev = get_existing_connection()
        scene_path = os.path.abspath(hlev.SNIFileName())
    ...
    def _syntheyes_find_additional_scene_dependencies(self):
        """
        Find additional dependencies from the scene
        """
        # initial implementation does nothing!
